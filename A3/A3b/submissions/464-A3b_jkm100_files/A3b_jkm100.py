...
def plotimpulseresponse(a=[-0.9], b=[0.1], fs=2000, duration=0.1, title="Impulse Response"):
    ...
    signal = np.zeros(int(duration*fs)+1)
    ipulseindex = int((fs*duration)/10)
    signal[ipulseindex] = 1

    filtered_signal = filterIIR(signal, a=a, b=b)

    plt.figure(figsize=(8,4), dpi=72)
    plt.axvline(t[ipulseindex-1], c=purples[3], label = "impulse")
    plt.plot(t, filtered_signal, c=purples[6], label="response")
